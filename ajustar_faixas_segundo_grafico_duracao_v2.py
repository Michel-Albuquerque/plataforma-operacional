...
MARCADOR = "FAIXAS_AMPLAS_GRAFICO_EQUIPE_DURACAO_V2"


BLOCO_ANTIGO = r"""    const layoutAssign=addTopTotalAnnotations({
    ...
    plot('chartRtTeamAssignmentBins',[
      {
        ...
        x:labels,
        y:grouped.map(g=>g.withoutTeam),
        marker:{color:grouped.map(g=>g.colors.a)},
        text:grouped.map(g=>g.withoutTeam>0?fmtInt(g.withoutTeam):''),
        textposition:'inside',
        textfont:{color:'#1f2937',size:11},
        customdata:grouped.map(g=>({key:g.key,label:g.label,segment:'SEM_EQUIPE'})),
      ...
      },
      {
        ...
        x:labels,
        y:grouped.map(g=>g.withTeam),
        marker:{color:grouped.map(g=>g.colors.b)},
        text:grouped.map(g=>g.withTeam>0?fmtInt(g.withTeam):''),
        textposition:'inside',
        textfont:{color:'#ffffff',size:11},
        customdata:grouped.map(g=>({key:g.key,label:g.label,segment:'COM_EQUIPE'})),
        hovertemplate:'<b>%{x}</b><br>Com equipe atribuída: %{y}<extra></extra>'
      }
    ],layoutAssign,ev=>{
      const data=ev.points[0].customdata;
      const group=grouped.find(g=>g.key===data.key);
      if(!group)return;
      const selected=data.segment==='COM_EQUIPE'?group.withTeamRows:group.withoutTeamRows;
      const prefix=group.key==='OVERDUE'?'Incidências que já atingiram 24h':`Faltam para 24h • ${group.label}`;
      const suffix=data.segment==='COM_EQUIPE'?'Com equipe atribuída':'Sem equipe atribuída';
      openDetails(`${prefix} • ${suffix}`,selected);
    });"""


BLOCO_NOVO = r"""    const assignmentDurationBands=[
      {
        key:'0-8',
        label:'0–8h',
...
def main() -> None:
    ...
    if BLOCO_ANTIGO not in text:
        raise RuntimeError(
            "O bloco do segundo gráfico não foi localizado. "
            "Confirme se o script empilhar_graficos_risco_bt_mt_e_equipe.py "
            "foi executado antes deste."
        )
    ...
    print("=" * 78)
    print("FAIXAS DO SEGUNDO GRÁFICO ATUALIZADAS")
    print("=" * 78)
